...
router.get('/', authenticated_1.authenticated, authorizedWithRole_1.authorizedWithRole('ROLE_ADMIN'), function (req, res) {
    attendeeService_1.attendeeService.getAttendees(true).then(function (teachers) {
        res.status(200);
...
router.post('/', function (req, res) {
    attendeeService_1.attendeeService.saveAttendee(req.body, true).then(function (userId) {
        res.status(201);
